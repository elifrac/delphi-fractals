...
const
  ...
  SFCantDelete = 'File %s is Read Only. Cannot Delete';
  SFOpenError = 'Cannot Open File';
  SFCreateError = 'Cannot Create File';

type
  EInvalidDest = class(EStreamError);
  EFCantMove = class(EStreamError);
  EFCantDelete = class(EStreamError);

procedure CopyFile2(const FileName, DestName: string);
...
begin
  ...
  begin
      CopyFile2(FileName, Destination); { copy it over to destination...}
       if HasAttr(FileName, faReadOnly) then  { if it's read-only... }
          raise EFCantDelete.Create(Format(SFCantDelete, [FileName])) { we wouldn't be able to delete it }
       else
